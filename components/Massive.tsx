...
const Massive: React.FC = () => {
  ...
  // Historical: "Resuelta" AND "Finalizada" as requested
  const historyIncidents = incidents.filter(i => 
      i.status === 'Resuelta' || 
      (i.status as string) === 'Finalizada' || // Handle legacy data
      i.recovery_status === 'Finalizada'
  );
                    ...
                    {activeTab === 'active' ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                            {filteredDisplay.map(incident => {
                                // Filter specific failures for this incident
