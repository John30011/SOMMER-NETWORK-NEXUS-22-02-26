        ...
        try {
            ...
            if (failData && failData.length > 0) {
                ...
                fetchedFailures = failData.map((f: any) => {
                    ...
                    const invMatch: any = invData?.find((i: any) => String(i.network_id || '').toUpperCase().trim() === fId);
                    return {
                        ...f,
                        nombre_tienda: invMatch?.nombre_tienda || f.nombre_tienda || f.network_id,
                        codigo_tienda: invMatch?.codigo_tienda || null,
                        meraki_url: invMatch?.meraki_url || null,
                        pais: invMatch?.pais || f.pais || 'Desconocido',
            ...
            <header className="h-auto md:h-16 border-b border-zinc-900 bg-black flex flex-col md:flex-row items-center justify-between px-4 md:px-8 py-3 md:py-0 sticky top-0 z-40 gap-3 md:gap-0">
                ...
                >
                    <div className={`relative w-full h-full transition-transform duration-700 transform-style-3d ${isHealthFlipped ? 'rotate-y-180' : ''}`}>
                        ...
                        <div
                            className={`absolute inset-0 backface-hidden rotate-y-180 rounded-lg px-3 py-1.5 border flex items-center justify-between gap-3 shadow-lg ${tpStatus === 'red'
                ...
                <div className="flex items-center gap-2 md:gap-4 w-full md:w-auto justify-end">
                    ...
                    <button
                        ...
                        className={`flex items-center justify-center p-1.5 rounded-full border transition-all ${autoVoiceEnabled
                                    ...
                                    {[
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`flex items-center gap-2 px-3 md:px-4 py-2 rounded-md text-xs md:text-sm font-medium transition-all duration-200 whitespace-nowrap ${isActive
                                            ...
                                            >
                                                ...
                                                {tab.label}
                                                <span className={`ml-1.5 px-1.5 py-0.5 rounded text-[10px] font-bold transition-colors ${tab.count > 0 && tab.id !== 'all'
