...
import { ISPProvider, ScheduledTask, DeviceInventory, ISPContact } from '../types';
import { Cable, Search, Plus, Trash2, Edit2, Globe, Phone, Mail, Save, X, Loader2, AlertCircle, Building2, ShieldCheck, CalendarClock, LayoutList, MapPin, Store, Timer, ChevronDown, Check, CalendarX, Clock, CheckSquare, Square, ChevronLeft, ChevronRight, Calendar, ChevronUp, BarChart, List, Users, Copy } from 'lucide-react';
                                ...
                                {Array.from({ length: daysInMonth }).map((_, i) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`h-8 w-8 rounded-full flex items-center justify-center text-xs transition-all ${isSelected ? 'bg-purple-600 text-white font-bold shadow-lg shadow-purple-900/50' :
...
const Providers: React.FC = () => {
    ...
    const [editingContacts, setEditingContacts] = useState<ISPContact[]>([]);
    const [activeContactDropdown, setActiveContactDropdown] = useState<{ index: number, type: 'level' | 'method' } | null>(null);
    const [flippedCardId, setFlippedCardId] = useState<number | string | null>(null);
    const [copiedTextId, setCopiedTextId] = useState<string | null>(null);
    const contactDropdownRef = useRef<HTMLDivElement>(null);

    // Click outside listener for Contact Dropdowns
    useEffect(() => {
    ...
    const fetchProviders = async () => {
        ...
        try {
            if (isDemoMode) {
                const mockProviders: ISPProvider[] = [
                    { id: 1, name: 'CANTV', sla_contract: 98.5, contacts: [{ provider_id: 1, level: 'Nivel 1', method: 'Correo', value: 'soporte@cantv.com.ve', contact_name: 'Mesa Corporativa' }] },
                    { id: 2, name: 'INTER', sla_contract: 99.0, contacts: [{ provider_id: 2, level: 'Nivel 1', method: 'Llamada', value: '0500-INTER-00', contact_name: 'Guardia NOC' }] },
                    { id: 3, name: 'MOVISTAR', sla_contract: 99.8, contacts: [{ provider_id: 3, level: 'Nivel 1', method: 'Llamada', value: '811', contact_name: 'Gerencia Cuentas' }] },
                    { id: 4, name: 'NETUNO', sla_contract: 97.5, contacts: [] },
                    { id: 5, name: 'CLARO', sla_contract: 99.5, contacts: [] },
                    { id: 6, name: 'ETB', sla_contract: 98.0, contacts: [] },
                    { id: 22, name: 'AIRTECK', sla_contract: 99.0, contacts: [] },
                ];
            ...
            const { data, error } = await supabase.from('isp_providers_jj').select('*, contacts:isp_contacts_jj(*)').order('name');
    ...
    const handleOpenModal = (provider?: ISPProvider) => {
        ...
        setEditingContacts(provider?.contacts ? [...provider.contacts] : []);
        setErrorMsg(null);
        setIsModalOpen(true);
    };
    ...
    const handleSaveProvider = async () => {
        ...
        try {
            ...
            // Remove joined contacts array or deprecated flat fields before updating the master table
            const { contacts, contact_name, contact_email, support_phone, ...providerPayload } = formData as any;

            if (editingProvider) {
                const { error } = await supabase.from('isp_providers_jj').update(providerPayload).eq('id', editingProvider.id);
                if (error) throw error;
            } else {
                const { data, error } = await supabase.from('isp_providers_jj').insert([providerPayload]).select();
                if (error) throw error;
                providerId = data[0].id;
            }

            // Sync Contacts
            if (!isDemoMode && providerId) {
                // First, delete the contacts the user removed from the UI.
                // We use the ID list of the contacts still present in the UI that have an ID.
                const validIds = editingContacts.map(c => c.id).filter(Boolean);
                if (validIds.length > 0) {
                    await supabase.from('isp_contacts_jj').delete().eq('provider_id', providerId).not('id', 'in', `(${validIds.join(',')})`);
                } else {
                    await supabase.from('isp_contacts_jj').delete().eq('provider_id', providerId);
                }

                // Then, process all remaining contacts (both existing edits & newly added)
                const validContacts = editingContacts.filter(c => c.value && c.contact_name);

                const existingContacts = validContacts.filter(c => c.id).map(c => ({
                    id: c.id,
                    ...
                    last_seen_api: new Date().toISOString()
                }));

                const newContacts = validContacts.filter(c => !c.id).map(c => ({
                    provider_id: providerId,
                    level: c.level,
                    method: c.method,
                    value: c.value,
                ...
                if (existingContacts.length > 0) {
                    const { error: updateError } = await supabase.from('isp_contacts_jj').upsert(existingContacts);
                    if (updateError) throw updateError;
                }

                if (newContacts.length > 0) {
                    const { error: insertError } = await supabase.from('isp_contacts_jj').insert(newContacts);
    ...
    const handleDeleteTaskContext = async (taskId: number) => {
        setIsTaskModalOpen(false);
        ...
        <div className="p-4 md:p-8 h-screen flex flex-col bg-black overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                ...
                <div className="flex bg-zinc-900 border border-zinc-800 rounded-lg p-1">
                    <button
                        ...
                        className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all flex items-center gap-2 ${activeTab === 'list'
                    ...
                    <button
                        ...
                        className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all flex items-center gap-2 ${activeTab === 'tasks'
                        ...
                        {loading ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                                {filteredProviders.map(provider => {
                                    ...
                                    return (
                                        <div key={provider.id} className="relative w-full group" style={{ perspective: '1200px', minHeight: '260px' }}>

                                            {/* Floor floating shadow */}
                                            <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 w-4/5 h-4 bg-purple-900/40 blur-[15px] rounded-[100%] scale-x-75 group-hover:scale-x-100 group-hover:bg-purple-600/50 transition-all duration-700 opacity-60 pointer-events-none z-0"></div>

                                            <div
                                                className={`w-full h-full transition-all duration-700 ease-[cubic-bezier(0.4,0,0.2,1)] relative ${flippedCardId === provider.id ? 'z-50' : 'z-10 hover:z-20'}`}
                                                style={{
                                                    transformStyle: 'preserve-3d',
                                                    transform: flippedCardId === provider.id ? 'translateY(-5px) rotateY(180deg) scale(1.03)' : 'translateY(0px) rotateY(0deg) scale(1)'
                                                }}
                                            >
                                                {/* FRONT FACE */}
                                                <div
                                                    className="absolute inset-0 w-full h-full flex flex-col justify-between bg-zinc-950 border border-zinc-900 rounded-xl p-5 shadow-[0_8px_30px_rgb(0,0,0,0.5)] cursor-pointer hover:border-zinc-800 transition-colors overflow-hidden"
                                                    style={{ backfaceVisibility: 'hidden' }}
                                                    onClick={() => setFlippedCardId(provider.id)}
                                                >
                                                    {/* Colored left stripe */}
                                                    <div className="absolute top-0 left-0 w-1.5 h-full bg-gradient-to-b from-purple-500 to-indigo-600 opacity-90"></div>
                                                    ...
                                                    <div className="pl-2 h-full flex flex-col justify-between">
                                                        <div>
                                                            <div className="flex justify-between items-start mb-4">
                                                                <div className="flex items-center gap-3">
                                                                    ...
                                                                    <div>
                                                                        <h3 className="text-white font-bold text-lg leading-none uppercase tracking-tight flex items-center gap-2 drop-shadow-md">
                                                                            {provider.name}
                                                                            {countries.length === 1 && (
                                                                                <span className="text-zinc-500 text-sm font-bold opacity-70">{getCountryCode(countries[0])}</span>
                                                                            )}
                                                                        </h3>
                                                            ...
                                                            <div className="mb-4 space-y-2">
                                                                {countries.length > 0 ? (
                                                                    countries.map(country => {
                                                                        const count = stats[country].size;
                                                                        const massiveThreshold = Math.round(count * 0.15);
                                                                        return (
                                                                            <div key={country} className="bg-zinc-900/50 rounded-lg p-3 border border-zinc-800/50 hover:bg-zinc-900/80 transition-colors">
                                                                                ...
                                                                                <div className="text-[10px] text-zinc-500">
                                                                                    Masiva en: <span className="text-zinc-300 font-bold">{massiveThreshold} sec.</span>
                                                                                </div>
                                                                            </div>
                                                                        );
                                                                    })
                                                        ...
                                                        {/* Bottom Info SLA */}
                                                        <div className="flex items-center justify-between mt-auto border-t border-zinc-900 pt-3">
                                                            <div className="flex items-center gap-1.5 opacity-80">
                                                                <ShieldCheck className="w-3.5 h-3.5 text-zinc-500" />
                                                                <span className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider">SLA</span>
                                                            </div>
                                                            <span className={`font-mono font-bold text-xs px-2 py-0.5 rounded ${provider.sla_contract && provider.sla_contract >= 99 ? 'bg-green-500/10 text-green-500' : 'bg-yellow-500/10 text-yellow-500'}`}>
                                                ...
                                                {/* BACK FACE */}
                                                <div
                                                    className="absolute inset-0 w-full h-full flex flex-col bg-zinc-950 border border-purple-500/50 rounded-xl p-5 shadow-[0_20px_50px_rgba(0,0,0,0.8)] cursor-pointer antialiased"
                                                    style={{ backfaceVisibility: 'hidden', transform: 'rotateY(180deg) translateZ(1px)' }}
                                                    onClick={() => setFlippedCardId(null)}
                                                >
                                                    <div className="flex justify-between items-center mb-4 shrink-0 border-b border-zinc-900 pb-2">
                                                        <h4 className="text-white font-bold text-xs flex items-center gap-2 uppercase tracking-wide">
                                                            <Users className="w-4 h-4 text-purple-400" />
                                                            Contactos <span className="text-zinc-500">({provider.contacts?.length || 0})</span>
                                                        </h4>
                                                        <button
                                                            onClick={(e) => { e.stopPropagation(); setFlippedCardId(null); }}
                                                            className="text-zinc-500 hover:text-white transition-colors bg-zinc-900 hover:bg-zinc-800 rounded p-1 shadow z-10 relative"
                                                        >
                                                            <X className="w-4 h-4" />
                                                        </button>
                                                    </div>

                                                    {/* Contacts Directory Scrollable Area */}
                                                    <div
                                                        className="flex-1 overflow-y-auto pr-1 space-y-2 custom-scrollbar pb-2 relative z-10"
                                                        onClick={(e) => e.stopPropagation()} /* Prevent flip when scrolling/clicking inside list */
                                                    >
                                                        {!provider.contacts || provider.contacts.length === 0 ? (
                                                            <div className="text-[10px] text-zinc-500 italic py-8 text-center bg-zinc-900/30 rounded-lg border border-dashed border-zinc-800/50 flex flex-col items-center gap-2">
                                                            ...
                                                            [...provider.contacts]
                                                                ...
                                                                .map((c, idx) => (
                                                                    <div key={idx} className="flex flex-col bg-zinc-900/60 border border-zinc-800/80 rounded p-2.5 transition-colors hover:bg-zinc-800/50">
                                                                        <div className="flex items-center justify-between mb-1.5">
                                                                            <span className="text-[10px] font-bold text-purple-400 capitalize">{c.level.toLowerCase()}</span>
                                                                            <span className="text-xs text-zinc-300 font-medium truncate max-w-[130px]" title={c.contact_name}>{c.contact_name}</span>
                                                                        </div>
                                                                        <div className="flex items-center gap-2 text-[11px]">
                                                                            <div className="bg-zinc-950 p-1 rounded border border-zinc-800 shadow-inner flex items-center justify-center shrink-0">
                                                                                {c.method === 'Correo' && <Mail className="w-3 h-3 text-zinc-400" />}
                                                                                {c.method === 'Llamada' && <Phone className="w-3 h-3 text-zinc-400" />}
                                                                                {c.method === 'Whatsapp' && <span className="text-[9px] font-bold text-green-500 leading-none">WA</span>}
                                                                            </div>
                                                                            <span className="truncate font-mono text-zinc-400 flex-1" title={c.value}>{c.value}</span>
                                                                            <button
                                                                                onClick={(e) => { e.stopPropagation(); handleCopy(c.value, `${provider.id}-${idx}`); }}
                                                                                className="p-1 rounded bg-zinc-900/80 hover:bg-purple-900/30 border border-zinc-800 hover:border-purple-500/50 text-zinc-500 hover:text-purple-400 transition-colors shrink-0 relative flex items-center justify-center"
                                                                                title="Copiar"
                                                                            >
                                                                                {copiedTextId === `${provider.id}-${idx}` && (
                                                                                    <span className="absolute -top-7 text-[9px] font-bold text-white bg-purple-600 px-1.5 py-0.5 rounded shadow shadow-purple-900/50 animate-in slide-in-from-bottom-1 fade-in duration-200 pointer-events-none whitespace-nowrap">
                                                                                        ¡Copiado!
                                                                                    </span>
                                                                                )}
                                                    ...
                                                    {/* Back Actions Footer */}
                                                    <div
                                                        className="pt-3 mt-auto border-t border-zinc-900 flex justify-between items-center shrink-0 relative z-10"
                                                        onClick={(e) => e.stopPropagation()}
                                                    >
                                                        <span className="text-[9px] text-zinc-600 font-mono tracking-widest pl-1">ID:{provider.id}</span>
                                                        <div className="flex gap-1.5">
                                                            <button
                                                                onClick={(e) => { e.stopPropagation(); handleOpenModal(provider); }}
                                                                className="px-3 py-1.5 rounded-md bg-purple-600/10 border border-purple-500/20 hover:bg-purple-600/30 hover:border-purple-500/50 text-purple-400 hover:text-white transition-all flex items-center gap-1.5 text-[10px] font-bold uppercase shadow-sm"
                                                            >
                                                                <Edit2 className="w-3 h-3" /> Editar
                                                            </button>
                                                            <button
                                                                onClick={(e) => { e.stopPropagation(); handleDeleteProvider(provider.id); }}
                                                                className="p-1.5 rounded-md bg-red-900/10 border border-red-900/20 hover:bg-red-900/40 text-red-500/80 hover:text-red-400 transition-all shadow-sm"
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-backdrop">
                    <div className="bg-zinc-950 border border-zinc-900 rounded-xl w-full max-w-lg shadow-2xl animate-smooth flex flex-col max-h-[90vh]">
                        ...
                        <div className="p-6 space-y-4 overflow-y-auto flex-1 custom-scrollbar">
                            ...
                            {/* DYNAMIC CONTACTS BUILDER */}
                            <div className="space-y-2 mt-4 border-t border-zinc-900 pt-4">
                                <div className="flex justify-between items-center mb-2">
                                    <label className="text-xs font-bold text-zinc-500 uppercase">Contactos Multinivel</label>
                                    <button
                                        type="button"
                                        onClick={() => setEditingContacts([...editingContacts, { provider_id: editingProvider?.id || 0, level: 'Nivel 1', method: 'Correo', value: '', contact_name: '' }])}
                                        className="text-xs bg-purple-600/20 text-purple-400 hover:bg-purple-600/40 px-2 py-1 rounded flex items-center gap-1 transition-colors"
                                    >
                                ...
                                <div className="space-y-2 pb-24">
                                    {editingContacts.length === 0 && (
                                        <div className="text-center p-3 border border-dashed border-zinc-800/50 rounded-lg text-zinc-500 text-xs">
                                            Sin contactos asignados. Haz click en Agregar Contacto.
                                        </div>
                                    )}
                                    {editingContacts.map((c, i) => (
                                        <div key={i} className="flex flex-col gap-2 p-3 bg-zinc-900/50 border border-zinc-800 rounded relative group animate-in fade-in">
                                            <button
                                                type="button"
                                                onClick={() => setEditingContacts(editingContacts.filter((_, index) => index !== i))}
                                            ...
                                            <div className="grid grid-cols-2 gap-2 pr-6">
                                                ...
                                                <div className="relative">
                                                    <button
                                                        type="button"
                                                        onClick={(e) => { e.stopPropagation(); setActiveContactDropdown({ index: i, type: 'level' }); }}
                                                        className="w-full flex items-center justify-between bg-zinc-900 border border-zinc-800 rounded p-1.5 focus:border-purple-500 outline-none text-xs text-white"
                                                    >
                                                        <span>{c.level}</span>
                                                        <ChevronDown className="w-3 h-3 text-zinc-500" />
                                                    </button>
                                                    {activeContactDropdown?.index === i && activeContactDropdown?.type === 'level' && (
                                                        <div
                                                            ref={contactDropdownRef}
                                                            className="absolute top-full left-0 w-full mt-1 bg-zinc-950 border border-zinc-800 rounded shadow-xl z-50 overflow-hidden"
                                                        >
                                                            {['Nivel 1', 'Nivel 2', 'Nivel 3', 'Nivel 4', 'Nivel 5'].map(l => (
                                                                <button
                                                                    key={l}
                                                                    type="button"
                                                                    onClick={() => { const nc = [...editingContacts]; nc[i].level = l; setEditingContacts(nc); setActiveContactDropdown(null); }}
                                                                    className={`w-full text-left px-3 py-2 text-xs hover:bg-zinc-900 transition-colors ${c.level === l ? 'text-purple-400 font-bold bg-zinc-900/50' : 'text-zinc-300'}`}
                                                                >
                                                                    {l}
                                                                </button>
                                                            ))}
                                                ...
                                                <div className="relative">
                                                    <button
                                                        type="button"
                                                        onClick={(e) => { e.stopPropagation(); setActiveContactDropdown({ index: i, type: 'method' }); }}
                                                        className="w-full flex items-center justify-between bg-zinc-900 border border-zinc-800 rounded p-1.5 focus:border-purple-500 outline-none text-xs text-white"
                                                    >
                                                        <span>{c.method}</span>
                                                        <ChevronDown className="w-3 h-3 text-zinc-500" />
                                                    </button>
                                                    {activeContactDropdown?.index === i && activeContactDropdown?.type === 'method' && (
                                                        <div
                                                            ref={contactDropdownRef}
                                                            className="absolute top-full left-0 w-full mt-1 bg-zinc-950 border border-zinc-800 rounded shadow-xl z-50 overflow-hidden"
                                                        >
                                                            {['Correo', 'Llamada', 'Whatsapp'].map(m => (
                                                                <button
                                                                    key={m}
                                                                    type="button"
                                                                    onClick={() => { const nc = [...editingContacts]; nc[i].method = m; setEditingContacts(nc); setActiveContactDropdown(null); }}
                                                                    className={`w-full text-left px-3 py-2 text-xs hover:bg-zinc-900 transition-colors ${c.method === m ? 'text-purple-400 font-bold bg-zinc-900/50' : 'text-zinc-300'}`}
                                                                >
                                                                    {m}
                                                                </button>
                                                            ))}
                                            ...
                                            <div className="grid grid-cols-2 gap-2">
                                                <input
                                                    ...
                                                    onChange={e => { const nc = [...editingContacts]; nc[i].contact_name = e.target.value; setEditingContacts(nc); }}
                                                    className="bg-zinc-900 border border-zinc-800 text-white text-xs rounded p-1.5 focus:border-purple-500 outline-none"
                                                />
                                                <input
                                                    type="text"
                                                    placeholder="Valor (Email/Tlf)"
                                                    value={c.value}
                                                    onChange={e => { const nc = [...editingContacts]; nc[i].value = e.target.value; setEditingContacts(nc); }}
                                                    className="bg-zinc-900 border border-zinc-800 text-white text-xs rounded p-1.5 focus:border-purple-500 outline-none"
                                                />
                                            </div>
                                        </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="space-y-1.5 relative" ref={countrySelectorRef}>
                                    ...
                                    <button
                                        ...
                                        className={`w-full flex items-center justify-between bg-zinc-900 border rounded p-2 pl-3 text-sm focus:outline-none transition-colors ${isCountrySelectorOpen ? 'border-purple-500 text-white' : 'border-zinc-800 text-white hover:border-zinc-700'
                                            }`}
                                    >
                                                    ...
                                                    displayedCountries.map(c => {
                                                        ...
                                                        return (
                                                            <div
                                                                ...
                                                                className={`flex items-center justify-between px-3 py-2 cursor-pointer rounded-lg text-xs transition-colors mb-0.5 ${isSelected ? 'bg-purple-600/10 text-white font-medium' : 'text-zinc-300 hover:bg-zinc-800'
                                                                    }`}
                                                            >
                                ...
                                <div className="space-y-1.5 relative" ref={providerSelectorRef}>
                                    ...
                                    <button
                                        ...
                                        className={`w-full flex items-center justify-between bg-zinc-900 border rounded p-2 pl-3 text-sm focus:outline-none transition-colors ${!taskFormData.tp_country
                                        ...
                                        {displayedStoresForTask.map(store => {
                                            ...
                                            return (
                                                <div
                                                    ...
                                                    className={`flex items-center gap-2 px-2 py-1.5 cursor-pointer rounded text-xs transition-colors ${isSelected ? 'bg-purple-600/20 text-white border border-purple-500/30' : 'hover:bg-zinc-800 text-zinc-400 border border-transparent'
                                                        }`}
                                                >
